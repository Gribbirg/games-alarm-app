...
- **`:feature:games:<name>`** — the other mini-games, one module each with the same structure as `calc` (fragment + ViewModel + pure-logic classes with unit tests + module README): `memory` («Повтори узор», id 2), `equation` («Уравнение», id 3), `sorting` («По порядку», id 4), `pairs` («Найди пару», id 5), `sequence` («Продолжи ряд», id 6), `stroop` («Цвет и слово», id 7), `oddoneout` («Найди лишнее», id 8), `maze` («Лабиринт», id 9), `anagram` («Анаграммы», id 10), `truefalse` («Верно или нет», id 11), `reaction` («Поймай момент», id 12), `counter` («Сосчитай», id 13), `lights` («Погаси свет», id 14), `fifteen` («Пятнашки», id 15), `chain` («Цепочка», id 16), `roman` («Римские числа», id 17), `clock` («Который час», id 18), `digits` («Запомни число», id 19), `hanoi` («Ханойская башня», id 20), `targetsum` («Набери сумму», id 21), `matrix` («Запомни клетки», id 22), `mole` («Поймай крота», id 23), `pattern` («Закономерность», id 24), `weekday` («День недели», id 25), `percent` («Проценты», id 26), `binary` («Двоичный код», id 27), `reverse` («Наоборот», id 28), `capitals` («Столицы», id 29), `spelling` («Как пишется?», id 30), `dice` («Кубики», id 31). Game ids must stay sequential 1..N — the hub indexes lists by `id - 1`.
