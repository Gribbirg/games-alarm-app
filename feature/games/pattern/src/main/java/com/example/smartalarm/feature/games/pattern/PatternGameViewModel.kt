...
/**
 * ViewModel экрана игры «Закономерность».
 *
 * Тонкий слой над чистой логикой [PatternGame]: ведёт таймер игры
 ...
 * а также считает итоговый счёт по формуле [computePatternScore].
 */
class PatternGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /** Создаёт новую игру [PatternGame] с текущей сложностью. */
    fun startGame() {
        game = PatternGame(difficulty, Random.Default)
    }

    /** Передаёт выбор варианта [optionIndex] в игру и возвращает её вердикт. */
    fun onOptionClicked(optionIndex: Int): AnswerResult = game.onOptionClicked(optionIndex)

    /**
     * Итоговый счёт: −10 за каждую ошибку плюс
     * (600 − прошло секунд) × сложность.
     */
    fun finishScore(): Int = computePatternScore(
        game.mistakes,
