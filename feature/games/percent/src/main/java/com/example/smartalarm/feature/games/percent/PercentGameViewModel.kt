...
/**
 * ViewModel игры «Проценты»: таймер, очки, перезапуск будильника
 * и тонкая обёртка над чистой логикой [PercentGame].
 *
 * Очки: −10 за каждую ошибку; при победе добавляется
 * (600 − прошло_секунд) × сложность, как в игре «Калькулятор».
 */
class PercentGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * Однократно создаёт партию [PercentGame] (после установки сложности)
     * и публикует первую задачу. Повторные вызовы (например, после
     * пересоздания фрагмента) игнорируются — партия продолжается.
     */
    fun startGame() {
        if (!::game.isInitialized) {
            game = PercentGame(difficulty)
            task.value = game.currentTask
            correctCount.value = game.correctCount
        }
    }

    /**
     * Обрабатывает выбор игроком варианта ответа: передаёт его в [PercentGame],
     * при ошибке снимает 10 очков и публикует новую задачу и прогресс.
     *
