...
/**
 * ViewModel экрана игры «Сосчитай».
 *
 * Тонкий слой над чистой логикой [CounterGame]: ведёт таймер игры
 ...
 * а также считает итоговый счёт по формуле [computeCounterScore].
 */
class CounterGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /** Создаёт новую игру [CounterGame] с текущей сложностью. */
    fun startGame() {
        game = CounterGame(difficulty, Random.Default)
    }

    /** Передаёт выбранный вариант ответа [option] в игру и возвращает её вердикт. */
    fun answer(option: Int): AnswerResult = game.answer(option)

    /**
     * Итоговый счёт: −10 за каждую ошибку плюс
     * (600 − прошло секунд) × сложность.
     */
    fun finishScore(): Int = computeCounterScore(
        game.mistakes,
