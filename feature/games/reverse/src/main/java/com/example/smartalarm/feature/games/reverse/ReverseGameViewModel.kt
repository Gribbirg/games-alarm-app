...
import com.example.smartalarm.feature.games.reverse.logic.ReverseGame
import kotlinx.coroutines.launch

/**
 * ViewModel экрана игры «Наоборот».
 *
 * Тонкий слой над чистой логикой [ReverseGame]: ведёт таймер игры,
 * считает очки, перезапускает будильник, если игра не пройдена,
 * и хранит состояние игры между пересозданиями фрагмента.
 */
class ReverseGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /** Штраф за ошибку (неверно выбранное слово): −10 очков. */
    fun mistake() {
        score -= 10
    }
