...
/**
 * ViewModel экрана игры «Столицы».
 *
 * Тонкий слой над чистой логикой [CapitalsGame]: ведёт таймер игры
 ...
 * а также считает итоговый счёт по формуле [computeCapitalsScore].
 */
class CapitalsGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    lateinit var game: CapitalsGame
        private set
    ...
    /** Создаёт новую игру [CapitalsGame] с текущей сложностью. */
    fun startGame() {
        game = CapitalsGame(difficulty, Random.Default)
    }

    /** Передаёт выбранный вариант [option] в игру и возвращает её вердикт. */
    fun onAnswer(option: String): AnswerResult = game.onAnswer(option)

    /**
     * Итоговый счёт: −10 за каждую ошибку плюс
     * (600 − прошло секунд) × сложность.
     */
    fun finishScore(): Int = computeCapitalsScore(
        game.mistakes,
