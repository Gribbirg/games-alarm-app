...
/**
 * ViewModel экрана игры «Кубики».
 *
 * Тонкий слой над чистой логикой [DiceGame]: ведёт таймер игры
 ...
 * а также считает итоговый счёт по формуле [computeDiceScore].
 * Состояние игры (раунд, текущий бросок) живёт здесь и переживает
 * поворот экрана: [startGame] создаёт [game] только один раз.
 */
class DiceGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /** Запоминает сложность (1..3) для создания игры и подсчёта очков. */
    fun setDifficultyLevel(difficulty: Int) {
        this.difficulty = difficulty
    }

    /**
     * Создаёт новую игру [DiceGame] с текущей сложностью, если она ещё
     * не создана. Повторный вызов (пересоздание фрагмента при повороте
     * экрана) игру не сбрасывает.
     */
    fun startGame() {
        if (!::game.isInitialized)
            game = DiceGame(difficulty, Random.Default)
    }

    /** Передаёт введённую игроком сумму [input] в игру и возвращает её вердикт. */
    fun answer(input: String): AnswerResult = game.answer(input)

    /**
     * Итоговый счёт: −10 за каждую ошибку плюс
     * (600 − прошло секунд) × сложность.
     */
    fun finishScore(): Int = computeDiceScore(
        game.mistakes,
