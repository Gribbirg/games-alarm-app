...
/**
 * ViewModel игры «Двоичный код»: таймер, очки, перезапуск будильника
 * и тонкая обёртка над чистой логикой [BinaryGame].
 *
 * Очки: −10 за каждую ошибку; при победе добавляется
 * (600 − прошло_секунд) × сложность, как в игре «Калькулятор».
 */
class BinaryGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * Обрабатывает выбор игроком варианта ответа: передаёт его в [BinaryGame],
