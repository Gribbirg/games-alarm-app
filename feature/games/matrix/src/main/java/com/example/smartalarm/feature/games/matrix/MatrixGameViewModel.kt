...
/**
 * ViewModel экрана «Запомни клетки».
 *
 * Тонкий слой над [MatrixGameLogic]: держит логику игры и текущую фазу
 * (показ/воспроизведение) — обе переживают пересоздание фрагмента при
 * повороте экрана, — ведёт секундный таймер и умеет перезапустить будильник,
 * если игра не пройдена (контракт экрана игры).
 */
class MatrixGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    lateinit var logic: MatrixGameLogic
        private set

    /**
     * `true` — фаза показа набора («Запоминай!»), `false` — фаза
     * воспроизведения («Повтори!»). После поворота экрана в фазе показа
     * фрагмент показывает набор заново на полное время.
     */
    var isShowPhase: Boolean = true
        private set
    ...
    fun initGame(difficulty: Int) {
        if (!::logic.isInitialized)
            logic = MatrixGameLogic(difficulty)
    }

    /** Помечает фазу показа набора (перед новым показом: старт раунда, ошибка). */
    fun markShowPhase() {
        isShowPhase = true
    }

    /** Помечает фазу воспроизведения (набор погашен, ход игрока). */
    fun markInputPhase() {
        isShowPhase = false
    }
     ...
     * `(600 − прошло_секунд) × сложность` (см. [MatrixGameLogic.finalScore]).
     */
    fun finishScore(): Int =
