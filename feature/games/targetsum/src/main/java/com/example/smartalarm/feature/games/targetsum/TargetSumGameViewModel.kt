...
/**
 * ViewModel экрана «Набери сумму».
 *
 * Тонкий слой над [TargetSumGame]: держит логику игры (раунд и выделенные
 * ячейки переживают пересоздание фрагмента, например поворот экрана),
 * ведёт секундный таймер и умеет перезапустить будильник, если игра
 * не пройдена (контракт экрана игры, см. `.claude/rules/games.md`).
 */
class TargetSumGameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    lateinit var game: TargetSumGame
        private set
     ...
     * (например, повороте экрана) партия продолжается с текущего раунда
     * и с сохранённым выделением.
     */
    fun initGame(difficulty: Int) {
        if (!::game.isInitialized)
            game = TargetSumGame(difficulty)
    }
    ...
    /**
     * Итоговый счёт: штрафы за неверные проверки плюс
     * `(600 − прошло_секунд) × сложность` (см. [TargetSumGame.finalScore]).
     */
    fun finishScore(): Int =
        game.finalScore((System.currentTimeMillis() - timeStarted) / 1000)
